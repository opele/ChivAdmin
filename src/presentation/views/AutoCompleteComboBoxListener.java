...
public class AutoCompleteComboBoxListener<T> {
	...
	public AutoCompleteComboBoxListener(ComboBox<T> comboBox, List<T> defaultItems) {
		...
		this.comboBox.setOnKeyReleased(event -> {
			...
			// this occurs if user types fast, the length of textfield will
			// record after the user has typed after a certain delay.
			...
			if (event.isControlDown() || event.getCode() == KeyCode.BACK_SPACE || event.getCode() == KeyCode.RIGHT || event.getCode() == KeyCode.LEFT
	...
	private void selectClosestResultBasedOnTextFieldValue(boolean affect, boolean inFocus) {
		...
		for (int i = 0; i < items.size(); i++) {
			if (items.get(i) != null && AutoCompleteComboBoxListener.this.comboBox.getEditor().getText() != null
					&& AutoCompleteComboBoxListener.this.comboBox.getEditor().getText().toLowerCase().equals(items.get(i).toString().toLowerCase())) {
				try {
		...
		if (!inFocus && comboBox.getEditor().getText() != null && comboBox.getEditor().getText().trim().length() > 0) {
			...
			// KeyEvent ke = new KeyEvent(comboBox, KeyCode.ENTER.toString(),
			// KeyCode.ENTER.getName(), KeyCode.ENTER.impl_getCode(), false,
			// false, false, false, KeyEvent.KEY_RELEASED);
			KeyEvent ke = new KeyEvent(KeyEvent.KEY_RELEASED, KeyCode.ENTER.toString(), KeyCode.ENTER.toString(), KeyCode.ENTER, false, false, false,
					false);
